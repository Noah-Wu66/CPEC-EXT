...
.ysp-qc-panel__agent-item.is-failed {
  ...
  class YspSecondaryQcApp {
    ...
    async stopCurrentJob() { if (!this.runtime.running) return; const checkpoint = this.getActiveCheckpoint(); let stoppedStatusText = '任务已结束，可以重新开始'; requestListJobAbort(this.runtime.listJobAbortToken); this.runtime.stopping = true; this.runtime.pauseRequested = false; this.runtime.statusText = stoppedStatusText; this.pushLog('任务已结束'); if (checkpoint) { checkpoint.status = 'stopped'; checkpoint.statusText = stoppedStatusText; const requestIds = uniqueTextList(this.getSecondaryQcInflightEntries(checkpoint).map((entry) => entry.requestId)); checkpoint.inflightEntries = []; await this.saveCheckpoint(); if (requestIds.length) await this.stopSecondaryQcRequests(requestIds); if (checkpoint.rows.length > 0) { const collectedRows = checkpoint.rows; const partialReport = { version: 1, startDate: checkpoint.startDate, endDate: checkpoint.endDate || checkpoint.startDate, targetCount: checkpoint.targetCount, actualCount: collectedRows.length, fileName: `二次质检 ${checkpoint.startDate === checkpoint.endDate ? checkpoint.startDate : `${checkpoint.startDate}-${checkpoint.endDate}`}`, rows: createSecondaryQcExportRows(collectedRows), generatedAt: new Date().toISOString() }; this.runtime.report = partialReport; await storageSetCached({ [STORAGE_KEYS.report]: partialReport }); await this.clearCheckpoint(); stoppedStatusText = `任务已结束，已收集 ${collectedRows.length} 条质检结果，可下载`; this.runtime.statusText = stoppedStatusText; this.pushLog(`已生成质检结果，共 ${collectedRows.length} 条，可下载`); } } this.runtime.running = false; this.runtime.jobType = ''; this.runtime.stopping = false; this.runtime.pauseRequested = false; this.render(); }
